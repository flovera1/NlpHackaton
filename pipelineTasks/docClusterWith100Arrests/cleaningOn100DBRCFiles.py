...
def words():
    allWords = None
    with open('allUnigrams.txt', 'r') as infile:
...
documentDict=dict()
for filename in os.listdir('../../data/tauExptData/hundSent'):
    if filename[-3:] == 'txt':
		#print(filename)
        with open(os.path.join('../../data/tauExptData/hundSent',filename),'r') as infile:
            documentDict[filename]=infile.read()

#print(documentDict['RVVB.A.0010.0002.pdf.txt'])
print("Cleaning....")
documents=[]
